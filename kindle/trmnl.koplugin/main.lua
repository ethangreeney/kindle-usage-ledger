...
TrmnlDisplay.default_settings = {
    api_key = nil,
    base_url = "http://192.168.1.10:8790"  -- your Mac's LAN address,
    refresh_interval = 30,
...
function TrmnlDisplay:init()
    ...
    -- This Kindle is a dedicated dashboard: always restore live display on launch.
...
function TrmnlDisplay:displayImage(image_path)
    ...
    -- A tap forces a fresh check but keeps the dashboard visible.
    self.image_widget.onTapRefresh = function()
        logger.info("TRMNL: Refreshing image via tap")
        self:fetchAndDisplay(true)
        return true
    end

    -- Keep taps available for refresh while preserving a reliable escape route.
    -- Holding the lower-left footer closes the dashboard until KOReader restarts.
    self.image_widget.onHoldExit = function()
        logger.info("TRMNL: Closing dashboard via footer hold")
        ...
        self.image_widget.ges_events = {
            ...
            HoldExit = {
                GestureRange:new {
                    ...
                    range = Geom:new {
                        x = 0,
                        y = math.floor(screen_height * 0.88),
                        w = math.floor(screen_width * 0.5),
                        h = math.ceil(screen_height * 0.12),
                    }
                }
            }
        }
...
function TrmnlDisplay:_performFetch()
    ...
    local image_path, image_changed = self:downloadImageIfNeeded(response)
    ...
    if not image_changed then
        logger.info("TRMNL: Screen unchanged, scheduling next check without redraw")
        NetworkMgr:afterWifiAction()
        self.retry_manager:reset()
        self:scheduleNextRefresh()
        self:validateAndFixScheduleState()
